...
function MobileBottomNavComponent() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 bg-transparent px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 lg:hidden">
      <div className="mx-auto grid max-w-lg grid-cols-4 gap-2 rounded-[28px] border border-slate-200/80 bg-white/92 p-2 shadow-[0_18px_45px_rgba(15,23,42,0.18)] backdrop-blur-2xl">
        {mobileNavigation.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`group relative flex flex-col items-center justify-center gap-1.5 rounded-2xl px-2 py-3 text-[11px] font-medium transition-all duration-300 ${
                isActive
            ...
            >
              {isActive ? (
                <span className="absolute inset-x-5 top-1 h-1 rounded-full bg-emerald-500/70" />
              ) : null}
